"""Seed data for the Research Radar database.

Contains mock data matching the Golden Contract schema for testing.
"""
...
from app.models import RadarAnalysis, Trend
...
def seed_database() -> None:
    ...
    Skips if data already exists for the seed date.
    """
    ...
    with get_db_context() as db:
        ...
        existing = db.query(RadarAnalysis).filter(
            RadarAnalysis.radar_date == SEED_DATA["radar_date"]
        ).first()

        if existing:
            print(f"Data already exists for {SEED_DATA['radar_date']}, skipping seed.")
            return

        # Create radar analysis record
        analysis = RadarAnalysis(
            radar_date=SEED_DATA["radar_date"],
            status="completed",
            trend_count=len(SEED_DATA["trends"]),
        )
        db.add(analysis)

        # Create trend records
        for trend_data in SEED_DATA["trends"]:
            trend = Trend(
                radar_date=SEED_DATA["radar_date"],
                ...
                signal_evidence=json.dumps(trend_data["signal_evidence"]),
                noise_indicators=json.dumps(trend_data["noise_indicators"]),
                architectural_verdict=trend_data["architectural_verdict"],
                timestamp=trend_data["timestamp"],
            )
            db.add(trend)

        print(f"Seeded database with {len(SEED_DATA['trends'])} trends for {SEED_DATA['radar_date']}")
